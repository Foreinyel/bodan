...
data = pd.read_csv('/Users/shihao/Documents/adagu-project/adagu_bodan/adagu_bodan/data_analysis/binaryClassification.csv',names=column_names)
...
def predict_win_no_win(bodan):
    x_column_names = column_names[0:25]
    X = np.array([bodan.__dict__[name] for name in x_column_names])
    X = ss.transform(X)
    y_predict_prob = lr.predict_proba(X)
    y_predict = lr.classes_[np.argmax(y_predict_prob > threshold, axis=1)]
    bodan.predict_win_no_win = y_predict[0]
    print y_predict[0]

for bodan in bodans:
    predict_win_no_win(bodan)
